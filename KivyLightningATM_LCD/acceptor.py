...
class CoinAcceptor:
    ...
    def setup_acceptor():
        ...
        # defines a setup with a specific pin, how this pin is configured (input or output) and the actual
        # position. is it open or closed. because a button is used, DOWN was chosen
        GPIO.setup(CoinAcceptor.button1Pin, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)

        #same as above
        GPIO.setup(CoinAcceptor.button2Pin, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)

    @staticmethod
    def set_button1_pressed():
        '''sets the pressed value in the shared values file to True, if button 1 was pressed'''

        # if button 1 is pushed, electricity flows and the value changes to True
        if GPIO.input(CoinAcceptor.button1Pin):
            # because it's only as long as the button is pushed, the state is saved in a variable in the
            # shared values file
            shared_values.SharedValues.button1_pressed = True

    @staticmethod
    def set_button2_pressed():
        '''sets the pressed value in the shared values file to True, if button 2 was pressed'''

        # if button 2 is pushed, electricity flows and the value changes to True
        if GPIO.input(CoinAcceptor.button2Pin):
            # because it's only as long as the button is pushed, the state is saved in a variable in the
            # shared values file
            shared_values.SharedValues.button2_pressed = True
